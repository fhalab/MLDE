...
from .finalize_x import finalize_x
from .train_and_predict import train_and_predict
from ..params.defaults import DEFAULT_TRAINING_PARAMS, DEFAULT_MODEL_PARAMS
...
def run_mlde(mlde_models, training_data, normalized_design_space,
             ...
             _debug = False, _reshape_x = False, _seeds = None):
    """
    ...
    for i, (model, train_test_inds) in tqdm(enumerate(model_args), 
        ...
        if _seeds is not None:
            ...
            # Grab te integer for the seed
            seed_int = _seeds[i]
            
            # Seed sklearn. Sklearn uses numpy random throughout.
            np.random.seed(seed_int)
            
            # Seed Keras. Keras uses random and tensorflow for seeding.
            if model.major_model == "Keras":
                random.seed(seed_int)
                tf.random.set_random_seed(seed_int)
